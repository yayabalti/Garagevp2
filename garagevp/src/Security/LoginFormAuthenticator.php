...
//     public const LOGIN_ROUTE = 'app_login';  // Route utilisée pour la page de connexion

//     public function __construct(
//         private LoggerInterface $logger,  // Service de log pour enregistrer les informations de connexion
//         private UrlGeneratorInterface $urlGenerator,  // Générateur d'URL pour rediriger après authentification
//         private EntityManagerInterface $entityManager,  // Manager d'entités pour interagir avec la base de données
//         private UserRepository $userRepository,  // Repository pour récupérer les utilisateurs de la base de données
//         private UserPasswordHasherInterface $passwordHasher  // Service pour vérifier les mots de passe hachés
//     ) {
//     }
...
//             'email' => $email,
//             'password_length' => strlen($password)  // Log de la longueur du mot de passe (pour des raisons de sécurité)
//         ]);
...
//     {
//         // Si l'authentification est réussie, on enregistre l'heure de dernière connexion
//         $user = $token->getUser();
//         if ($user instanceof User) {
//             $this->logger->info('Connexion réussie', [
//                 'email' => $user->getEmail(),
//                 'roles' => $user->getRoles()  // Log des rôles de l'utilisateur pour audit
//             ]);
//             $user->setLastLoginAt(new \DateTimeImmutable());
//             $this->entityManager->flush();  // Sauvegarde les informations de l'utilisateur
//         }

//         // Redirection vers la page précédente ou vers la page d'accueil
...
//     {
//         // Si l'authentification échoue, on log l'erreur et on met à jour l'état de l'utilisateur
//         $this->logger->warning('Échec de connexion', [
//             'message' => $exception->getMessage()  // Message d'erreur pour aider à l'audit
//         ]);

//         // On enregistre le nombre d'échecs et on verrouille l'utilisateur si nécessaire
//         $email = $request->request->get('email', '');
//         $user = $this->userRepository->findOneBy(['email' => $email]);
...
//             // Si l'utilisateur dépasse 3 échecs, son compte est verrouillé
...
//         // Renvoie la réponse d'échec d'authentification
